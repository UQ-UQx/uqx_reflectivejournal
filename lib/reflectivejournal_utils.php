...
function get_tagcloud($journalentries)
{
  ...
  {
    $cloud->addString(strip_tags_with_whitespace(html_entity_decode($entry['reflectivetext'])));
  }
  return $cloud->render();
}
...
function get_journalentries($db, $student_id){
    $select = $db->query( 'SELECT * FROM studentresponse WHERE student_id = :student_id', array( 'student_id' => $student_id  ) );
